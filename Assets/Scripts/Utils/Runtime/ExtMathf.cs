...
namespace utils.runtime
{
    public static class ExtMathf
    {
        ...
        public static float Remap(this float value, float from1, float to1, float from2, float to2, AnimationCurve animationCurve)
        {
            float remappedValue = (value - from1) / (to1 - from1) * (to2 - from2) + from2;

            float percentRemap = (value - from1) / (to1 - from1) * (1 - 0) + 0;
            float evaluate = animationCurve.Evaluate(percentRemap);

            float finalEvaluated = (evaluate - 0) / (1 - 0) * (to2 - from2) + from2;
        ...
        /// <returns></returns>
        public static float SetBetween(this float currentValue, float value1, float value2)
        {
            if (value1 > value2)
            {
        ...
        /// get closest point from an array of points
        /// </summary>
        public static Vector3 GetClosestPointArray(Vector3 posEntity, List<Vector3> arrayPos, out int indexFound)
        {
            float sqrDist = (posEntity - arrayPos[0]).sqrMagnitude;
            indexFound = 0;

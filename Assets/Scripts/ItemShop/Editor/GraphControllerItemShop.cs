...
{
	public class GraphControllerItemShop : EditorWindow
	{
		public static GraphControllerItemShop _current;
		private ItemShop _target;
		private GraphSidebar _sidebar;
		public GraphCameraItemShop _camera;
		...
		private ItemCollection _selectNode; // Actively selected node for a transition

		private ItemCollection _snapNode; // We need to refresh this node and force snap it
		public bool _forceSnapping = true;

		private void OnEnable()
		{
			titleContent.text = "Item Shop";
			...
			if (_camera == null) _camera = new GraphCameraItemShop();
		...
		[MenuItem("Window/Item Shop")]
		private static void ShowEditor()
		{
			EditorWindow.GetWindow<GraphControllerItemShop>();
		}
		...
		private void UpdateTarget(GameObject go)
		{
			if (go != null)
			{
				...
				if (itemShop)
				{
					// Verify inheritance on ItemShop
					if (IsValidInheritance(itemShop))
					{
						// Assign the new item shop
						_target = itemShop;
					...
					{
						Debug.LogError("Invalid inheritance for item classes. Please verify they inherit from proper base classes before proceeding.");
					}
				}
			}
		...
		private bool IsValidInheritance(ItemShop target)
		{
			/*
		...
		private void OnGUI()
		{
			...
			// Poll the current item collection
			if (_target._currentCategory != null)
			{
				_collect = _target._currentCategory.GetComponentsInChildren<ItemCollection>();
			}
			else
			{
				_collect = new ItemCollection[0];
			}
					...
					if (e.button == 1 && !_isTransition) // RIGHT CLIC
					{
						if (e.type == EventType.MouseDown)
						{
							...
							if (clickedNode)
							{
								...
								menu.AddItem(new GUIContent("Add Item"), false, AddItem, _collect[_selectIndex]);
								//menu.AddItem(new GUIContent("Add Child Transition"), false, BeginSkillGroupTransition);
								menu.AddSeparator("");
								menu.AddItem(new GUIContent("Delete Item Group"), false, DeleteItemGroup);
								menu.AddSeparator("");

								/*
								foreach (ItemCollection child in _collect[_selectIndex].ChildSkills)
								{
							...
							{
								GenericMenu menu = new GenericMenu();
								menu.AddItem(new GUIContent("Add Item Group"), false, CreateItemGroup);
								menu.ShowAsContext();
								e.Use();
							}
						}
		...
		public void SnapNodeToGrid(ItemCollection node)
		{
		...
		{
			foreach (ItemCollection col in _collect)
			{
				SnapNodeToGrid(col);
			}
		}
		...
		private void DrawNodeWindow(int id)
		{
			...
			foreach (Item item in _collect[id].GetComponentsInChildren<Item>())
			{
				GUILayout.BeginHorizontal();
				item.Unlocked = GUILayout.Toggle(item.Unlocked, "");

				if (GUILayout.Button(item.displayName, GUILayout.Width(100f)))
				{
			...
			if (deleteItem != null) DestroyImmediate(deleteItem.gameObject);
		...
		private void CreateItemGroup()
		{
			...
			go.name = "ItemCollection";
			ItemCollection skill = go.AddComponent(_target.ItemCollection) as ItemCollection;
		...
		{
			if (EditorUtility.DisplayDialog("Delete Item Collection?",
											"Are you sure you want to delete this item collection? It will delete this collection plus all items it contains.",
											"Delete Item Collection",
											"Cancel"))
			{
				ItemCollection[] collect = _target._currentCategory.GetComponentsInChildren<ItemCollection>();
				ItemCollection t = collect[_selectIndex];
		...
		{
			ItemCollection col = obj as ItemCollection;
			AddItem(col);
		}

		private void AddItem(ItemCollection col)
		{
			GameObject go = new GameObject();
			go.name = "Item";
			Item s = go.AddComponent(_target.Item) as Item;

	...
	public class GraphSidebar
	{
		...
		public void DrawSidebar(Rect rect, float padding, Color color)
		{
			...
			if (_target != null)
			{
				...
				{
					ItemCategory cat = child.GetComponent<ItemCategory>();
					...
					if (GUI.Button(new Rect(24f, 0f, innerWidth - 82f, 20f), cat.displayName))
					{
						...
						GraphControllerItemShop._current._camera.Reset();
					}
				...
				if (GUI.Button(new Rect(0f, y, innerWidth, 20f), "Create Category"))
				{
					...
					go.name = "Category";
					go.AddComponent(_target.ItemCategory);
				...
				{
					GraphControllerItemShop._current.SnapAllNodesToGrid();
				}

				y += 25f;

				GraphControllerItemShop._current._forceSnapping = GUI.Toggle(new Rect(0f, y, innerWidth, 20f), GraphControllerItemShop._current._forceSnapping, "Force Snapping");
			}

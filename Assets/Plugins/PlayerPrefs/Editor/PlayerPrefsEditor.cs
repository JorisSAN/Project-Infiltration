...
namespace Sabresaurus.PlayerPrefsExtensions
{
    public class PlayerPrefsEditor : EditorWindow
    {
        ...
        [MenuItem("Plugins/PlayerPrefs Editor", priority = 100)]
        ...
        private void UpdateSearch()
        {
            ...
            for (int i = 0; i < entryCount; i++)
            {
                ...
                bool isEncryptedPair = PlayerPrefUtility.IsEncryptedKey(deserializedPlayerPrefs[i].Key);
                if (automaticDecryption && isEncryptedPair)
                {
                    displayKey = PlayerPrefUtility.DecryptKey(fullKey);
                }
        ...
        private void DrawMainList()
        {
            ...
            for (int i = firstShownIndex; i < shownIndexLimit; i++)
            {
                ...
                bool isEncryptedPair = PlayerPrefUtility.IsEncryptedKey(activePlayerPrefs[i].Key);
                ...
                if (isEncryptedPair && automaticDecryption)
                {
                    ...
                    try
                    {
                        deserializedValue = PlayerPrefUtility.GetEncryptedValue(fullKey, (string)deserializedValue);
                        displayKey = PlayerPrefUtility.DecryptKey(fullKey);
                    }
                    catch
                    {
                ...
                if (isEncryptedPair && automaticDecryption && !failedAutoDecrypt)
                {
                    ...
                    if (encryptedValue.StartsWith(PlayerPrefUtility.VALUE_FLOAT_PREFIX))
                    {
                        valueType = typeof(float);
                    }
                    else if (encryptedValue.StartsWith(PlayerPrefUtility.VALUE_INT_PREFIX))
                    {
                        valueType = typeof(int);
                    }
                    else if (encryptedValue.StartsWith(PlayerPrefUtility.VALUE_BOOL_PREFIX))
                    {
                        valueType = typeof(bool);
                    }
                    else if (encryptedValue.StartsWith(PlayerPrefUtility.VALUE_STRING_PREFIX) || string.IsNullOrEmpty(encryptedValue))
                    {
                ...
                if (valueType == typeof(float))
                {
                    ...
                    if (isEncryptedPair && automaticDecryption)
                    {
                        ...
                        initialValue = PlayerPrefUtility.GetEncryptedFloat(displayKey);
                    }
                    else
                    {
                    ...
                    if (newValue != initialValue)
                    {
                        ...
                        if (isEncryptedPair)
                        {
                            string encryptedValue = PlayerPrefUtility.VALUE_FLOAT_PREFIX + SimpleEncryption.EncryptFloat(newValue);
                ...
                else if (valueType == typeof(int)) // if we're dealing with an int
                {
                    ...
                    {
                        // Automatically decrypt the value if encrypted and in auto-decrypt mode
                        initialValue = PlayerPrefUtility.GetEncryptedInt(displayKey);
                    }
                    else
                    {
                        // Otherwise fetch the latest plain value from PlayerPrefs in memory
                    ...
                    if (newValue != initialValue)
                    {
                        ...
                        if (isEncryptedPair)
                        {
                            string encryptedValue = PlayerPrefUtility.VALUE_INT_PREFIX + SimpleEncryption.EncryptInt(newValue);
                            SetString(fullKey, encryptedValue);
                        }
                        else
                        {
                ...
                else if (valueType == typeof(bool)) // if we're dealing with a bool
                {
                    ...
                    {
                        // Automatically decrypt the value if encrypted and in auto-decrypt mode
                        initialValue = PlayerPrefUtility.GetEncryptedBool(displayKey);
                    }
                    else
                    {
                        // Otherwise fetch the latest plain value from PlayerPrefs in memory
                    ...
                    if (newValue != initialValue)
                    {
                        ...
                        if (isEncryptedPair)
                        {
                            string encryptedValue = PlayerPrefUtility.VALUE_BOOL_PREFIX + SimpleEncryption.EncryptBool(newValue);
                            SetString(fullKey, encryptedValue);
                        }
                        else
                        {
                ...
                else if (valueType == typeof(string)) // if we're dealing with a string
                {
                    ...
                    {
                        // Automatically decrypt the value if encrypted and in auto-decrypt mode
                        initialValue = PlayerPrefUtility.GetEncryptedString(displayKey);
                    }
                    else
                    {
                        // Otherwise fetch the latest plain value from PlayerPrefs in memory
                    ...
                    if (newValue != initialValue && !failedAutoDecrypt)
                    {
                        ...
                        if (isEncryptedPair)
                        {
                            string encryptedValue = PlayerPrefUtility.VALUE_STRING_PREFIX + SimpleEncryption.EncryptString(newValue);
                            SetString(fullKey, encryptedValue);
                        }
                        else
                        {
        ...
        private void DrawAddEntry()
        {
            ...
            if ((GUILayout.Button("Add", GUILayout.Width(40)) || keyboardAddPressed) && !string.IsNullOrEmpty(newEntryKey))
            {
                ...
                {
                    // Encrypt the key
                    string encryptedKey = PlayerPrefUtility.KEY_PREFIX + SimpleEncryption.EncryptString(newEntryKey);
                    ...
                    if (newEntryType == PlayerPrefType.Float)
                    {
                        encryptedValue = PlayerPrefUtility.VALUE_FLOAT_PREFIX + SimpleEncryption.EncryptFloat(newEntryValueFloat);
                    }
                    else if (newEntryType == PlayerPrefType.Int)
                    {
                        encryptedValue = PlayerPrefUtility.VALUE_INT_PREFIX + SimpleEncryption.EncryptInt(newEntryValueInt);
                    }
                    else if (newEntryType == PlayerPrefType.Bool)
                    {
                        encryptedValue = PlayerPrefUtility.VALUE_BOOL_PREFIX + SimpleEncryption.EncryptBool(newEntryValueBool);
                    }
                    else
                    {
                        encryptedValue = PlayerPrefUtility.VALUE_STRING_PREFIX + SimpleEncryption.EncryptString(newEntryValueString);
                    }
